...
class Gcal {
	...
	private function _initAuth($configObj) {
		...
		if ($this->CI->input->get_post('code')) {
			...
			redirect($configObj['redirectURI']); //08-FEBRERO-2016
			#redirect('https://redisoftsystems.com/nube/google/welcome/api');
			#redirect('http://localhost/google/welcome/api');
		}
	...
	public function listCalendarList($configObj = array()) {
		...
		if (!isset($configObj['redirectURI'])) {
			...
			$this->apiClient->setRedirectUri('https://' . $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF']);
			
			#$this->apiClient->setRedirectUri('https://redisoftsystems.com/nube/google/welcome/api');
			#$this->apiClient->setRedirectUri('http://localhost/google/welcome/api');
		}
	...
	public function eventUpdate($configObj = array()) 
	{
		...
		$postBody->colorId 			= $configObj['colorId'];
	...
	public function eventInsert($configObj = array()) {
		...
		if ($configObj['allday']) 
		{
			...
			$start->setDateTime('2016-02-23T09:25:00');
			$postBody->setStart($start);
			
			$end = new EventDateTime();
			$end->setDateTime('2016-02-23T10:25:00');
			$postBody->setEnd($end);*/
		...
		else 
		{
			...
			#echo date('Y-m-d', $configObj['start']) . "T" . date('H:i:s', $configObj['start']);
