<script type="text/javascript" src="<?php echo base_url()?>js/configuracion/procesos.js"></script>
<div class="derecha">
<div class="submenu">
    ...
    <!--<div class="seccionDiv">
      ...
      <table class="toolbar" width="100%">
        <tr>
			...
			?>
            <td width="5%" align="center" valign="middle"><a href="<?php print(base_url()."configuracion/procesos"); ?>" class="escalaGrisesConfiguracion"><span class="icon-option" title="Estilo"><img src="<?php print(base_url()); ?>img/produccion.png"  width="30" height="34" title="Procesos"  /></span>Procesos</a></td>
 ...
 <table class="toolbar" width="10%">
    <tr>
      <td style="border:none" width="27%" align="center" valign="middle" class="button">
      	<a id="agregarProceso" onclick="formularioProcesos()" title="Agregar proceso">
           <img src="<?php print(base_url()); ?>img/add.png" border="0" title="Agregar proceso" /> <br />
           Agregar
        </a>
       ...
       <?php
		if($permiso[1]->activo==0)
		{
			 echo '
			<script>
				desactivarBotonSistema(\'agregarProceso\');
			</script>';
		}
       ?>
       </td>
...
if(!empty ($procesos))
{
    ...
    <thead>
     <tr >
        ...
        <th class="encabezadoPrincipal" width="70%" align="center">Proceso de producción</th>
...
foreach ($procesos as $row)
{
	...
	<td align="center" valign="middle">
    	<img id="btnEditarProceso<?php echo $i?>" src="<?php echo base_url()?>img/editar.png" width="22" height="22" title="Editar" onClick="accesoEditarProceso(<?php echo $row->idProceso?>)" >
        &nbsp;&nbsp;&nbsp;
         <img id="btnBorrarProceso<?php echo $i?>" src="<?php echo base_url()?>img/borrar.png" width="22" height="22" title="Borrar" onClick="borrarProceso(<?php echo $row->idProceso?>,'¿Esta seguro de borrar este proceso?')" >
        
        <br />
        
        
        <a id="a-btnEditarProceso<?php echo $i?>">Editar</a>
        <a id="a-btnBorrarProceso<?php echo $i?>">Borrar</a>
		 ...
		 <?php
			if($permiso[2]->activo==0)
			{
				 echo '
				<script>
					desactivarBotonSistema(\'btnEditarProceso'.$i.'\');
				</script>';
			}
			...
			if($permiso[3]->activo==0)
			{
				 echo '
				<script>
					desactivarBotonSistema(\'btnBorrarProceso'.$i.'\');
				</script>';
			}
    	?>
	</td>
...
}else
{
    print('<div class="Error_validar" style="margin-top:2px; width:90%; margin-bottom: 5px;">
           Sin registros de procesos de producción.
           </div>');
}
?>

<div id="ventanaAgregarProceso" title="Procesos de producción">
<div style="width:99%;" id="registrandoProceso"></div>
<div id="ErrorRegistrarProceso" class="ui-state-error" ></div>
	<script>
    $(document).ready(function()
    {
        $("#txtNombreProceso").autocomplete(
        {
            source:base_url+'configuracion/obtenerProcesosRepetidos',
            
            select:function( event, ui)
            {
                notify("El proceso ya esta registrado",500,5000,"error",5,5);
                document.getElementById("txtNombreProceso").reset();
            }
        });
    });
    </script>
...
<table class="admintable" width="100%;">
    <tr>
        ...
        <td>
        	<input name="txtNombreProceso" id="txtNombreProceso" type="text" class="cajas" style="width:250px" />
        </td>
    </tr>
</table>
</div>

<div id="ventanaEditarProceso" title="Editar proceso">
<div id="editandoProceso"></div>
<div id="ErrorRegistrarProceso" class="ui-state-error" ></div>
<div id="obtenerProceso"></div>
</div>
