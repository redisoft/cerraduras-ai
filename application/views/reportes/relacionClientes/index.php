<script src="<?php echo base_url()?>js/reportes/relacionClientes.js"></script>	
<script src="<?php echo base_url()?>js/tablero.js"></script>

<script>
$(document).ready(function()
{
	obtenerRelacionClientes();
	
	$("#txtMes").monthpicker();
	
});

</script>

<div class="derecha">
<div class="submenu">
<div class="breadcumb"><?php echo isset($breadcumb)?$breadcumb:''?></div>
<div class="toolbar" id="toolbar" >
<!--<div class="seccionDiv">
	Relación clientes
</div>-->
    <table class="toolbar" border="0" width="100%">
        <tr>
        	<td width="70%" align="left" valign="middle" style="border:none";>
                <select class="cajas" id="selectAnio" style="width:120px" onchange="obtenerRelacionClientes()">  
                	<option value="0">Seleccione año</option>
					<?php
                    for($i=2010;$i<=2100;$i++)
                    {
                        echo '<option value="'.$i.'">'.$i.'</option>';
                    }
                    ?>
                </select>
                
                 <select class="cajas" id="selectEmisores" name="selectEmisores" style="width:400px" onchange="obtenerRelacionClientes()">
                	<option value="0">Seleccione emisor</option>
                    <?php
                    foreach($emisores as $row)
					{
						echo '<option value="'.$row->idEmisor.'">('.$row->rfc.')'.$row->nombre.'</option>';
					}
					?>
                </select>
        	</td>  
        </tr>
    </table> 
</div>       
</div>
       
<div class="listproyectos">
	<div id="generandoReporte"></div>
	<div id="obtenerRelacionClientes"></div>
</div>

</div>



<div id="ventanaFacturasTablero" title="Facturas:">
<div class="ui-state-error" ></div>
<div id="cargarDetallesFactura"></div>
</div>
