	...
	foreach($ventas as $row)
	{
		...
		if($cancelada==0)
		{
			...
			<tr '.$estilo.'>
				...
				<td width="14%" align="center">'.$row->empresa.'</td>
				<td width="6%" align="left">'.$row->ordenCompra.' '.($row->idTienda>0?'('.$row->tienda.')':'').'</td>
				
				<td width="6%" align="left">'.$row->estacion.'</td>
				
				<td width="8%" align="left">'.$row->identificador.'</td>
				<td width="8%" align="left">'.$row->usuario.'</td>';
				...
				<td width="7%" align="center">'.$row->formaPago.'</td>
