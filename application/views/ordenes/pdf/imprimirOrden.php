...
<table class="admintable" width="100%" style="margin-top:3px">
	<tr>
		<th colspan="2" class="encabezadoPrincipal">Detalles de producto</th>
	</tr>
	<tr>
	...
	<tr>
		...
		<td>
			'.round($cantidadOrden,decimales).'
			<input type="hidden" class="cajas" id="txtCantidadProduccion" name="txtCantidadProduccion" value="'.$cantidadOrden.'"  />
		</td>
	</tr>
</table>
...
foreach($materiales as $row)
{
	...
	<tr '.($i%2>0?'class="sinSombra"':'class="sombreado"').'>
		...
		<td>';
			...
			{
				echo $c==1?'
				<label>'.$compra->nombre.'</label>, Disponible: '.number_format($compra->disponible,4):'
				<br />
				<label>'.$compra->nombre.'</label>, Disponible: '.number_format($compra->disponible,4);
