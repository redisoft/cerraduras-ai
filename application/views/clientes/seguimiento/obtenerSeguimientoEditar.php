...
<table class="admintable" width="100%;">
	...
	<tr>
		...
		<td>
			<select id="selectAreas" name="selectAreas" class="cajas" onchange="obtenerConceptosArea()" style="width:300px; ">';
			
			foreach($areas as $row)
			{
				echo '<option '.($row->idArea==$seguimiento->idArea?'selected="selected"':'').' value="'.$row->idArea.'">'.$row->nombre.'</option>';
			}
	...
	<tr>
		...
		<td id="obtenerConceptosArea">
			...
			{
				echo '<option '.($row->idConcepto==$seguimiento->idConcepto?'selected="selected"':'').' value="'.$row->idConcepto.'">'.$row->nombre.'</option>';
			}
