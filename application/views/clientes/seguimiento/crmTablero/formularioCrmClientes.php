...
$(document).ready(function()
{
	...
	$("#txtBuscarClienteCrm").autocomplete(
	{
		source:base_url+'configuracion/obtenerClientesActivos/0',
		
		select:function( event, ui)
		{
			$('#txtIdCliente').val(ui.item.idCliente);
			obtenerContactosCliente(ui.item.idCliente);
		}
	});
	
	$("#txtBuscarCotizacionCrm").autocomplete(
	{
		source:base_url+'configuracion/obtenerListaCotizaciones/0',
		
		select:function( event, ui)
		{
			$('#txtIdCotizacionCrm').val(ui.item.idCotizacion);
			$('#txtIdClienteBusquedaCrm').val(ui.item.idCliente);
		}
	});
	
	$("#txtBuscarVentaCrm").autocomplete(
	{
		source:base_url+'configuracion/obtenerListaVentas/0',
...
<form id="frmSeguimiento" name="frmSeguimiento">
    ...
    <table class="admintable" width="100%;">
        ...
        <tr>
            ...
            <td>
            	...
            	<div id="obtenerStatusCrm" style="width:220px; float: left">
					...
					if(sistemaActivo=='IEXE') $idStatus=4;
					
                    foreach($status as $row)
                    {
                        echo '<option '.($idStatus==$row->idStatus?'selected="selected"':'').' value="'.$row->idStatus.'|'.($row->idStatus<=4?$row->idStatus:$row->idStatusIgual).'">'.$row->nombre.'</option>';
                    }
                    ?>
                    </select>
                </div>
            ...
            <td>
            	...
            	<div id="obtenerEstatusCrm" style="width:220px; float: left">
                    <select id="selectEstatus" name="selectEstatus" class="cajas" style="width:200px; ">
                        ...
                        {
                            echo '<option '.(2==$row->idEstatus?'selected="selected"':'').'  value="'.$row->idEstatus.'">'.$row->nombre.'</option>';
                        }
                        ?>
                    </select>
                </div>
        ...
        <tr>
            ...
            <td>                           <!-- FECHA CON HORA -->
                <input type="text" name="txtFechaSeguimiento" id="txtFechaSeguimiento" class="cajas" style="width:100px;" value="<?php echo $fecha?>"  /> 
                
                Hora
                <input type="text" name="txtHoraSeguimiento" id="txtHoraSeguimiento" class="cajas" style="width:40px;" value="<?php echo $hora1?>" readonly="readonly"  />
        ...
        <tr id="filaServicio">
            ...
            <td>
                ...
                <img onclick="obtenerCatalogoServicios()" src="<?php echo base_url()?>img/agregar.png" width="20" height="20" title="Servicios" style="float:left; margin-top:-10px" />
                ...
                <input type="hidden" name="txtIdClienteBusquedaCrm" 	id="txtIdClienteBusquedaCrm" 	value="0" />
                
            </td>
        </tr>
        
        <tr>
            <td class="key"><?php echo sistemaActivo=='IEXE'?'Cliente/Alumno':''?>:</td>
            <td>                       
                <input type="text" name="txtBuscarClienteCrm" id="txtBuscarClienteCrm" class="cajas cajasSeleccionar" style="width:500px;" placeholder="Seleccione"  /> 
        ...
        <tr id="filaCierre">
            ...
            <td>                    
                ...
                <input type="text" name="txtFechaCierre" id="txtFechaCierre" class="cajas" style="width:100px;" value="<?php echo $fecha?>"/> 
                
                Hora
                <input type="text" name="txtHoraCierre" id="txtHoraCierre" class="cajas" style="width:40px;" value="<?php echo $hora1?>" readonly="readonly"  />
