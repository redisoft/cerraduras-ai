...
<head>
	...
	<script>
		...
		{
			JsBarcode("#codigoTicket", "'.$venta->folio.'");
		});
	</script>';
	?>
    <style>
...
<?php
	...
	<table style="font-size:18px" width="100%">
		...
		<tr>
			<th style="font-weight:normal" align="left" colspan="3">Descripción</th>
		</tr>
		<tr>
			<th style="font-weight:normal" align="left" colspan="3">Clave</th>
		</tr>
		<tr>
			<th style="width:15%; font-weight:normal">Cant.</th>
			<th style="width:23%; font-weight:normal">PU</th>
			<th style="width:23%; font-weight:normal" colspan="">Importe</th>
		</tr>';
		...
		foreach($productos as $row)
		{
			...
			echo'
			<tr>
				<td colspan="3">'.$row->nombre.($row->idPedimento>0?'<br />Pedimento: '.$row->pedimento.', Fecha: '.$row->fecha:'').'</td>
			</tr>
			<tr>
				<td colspan="3">'.$row->codigoInterno.'</td>
			</tr>
			<tr>
				<td align="center">'.number_format($row->cantidad,2).'</td>
				<td align="center">$'.number_format($row->precio+$impuesto,2).'</td>
				<td align="right" >$'.number_format($row->importe+$row->impuestos,2).'</td>
			</tr>';
		}
