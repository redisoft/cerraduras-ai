...
<script type="text/javascript">
	...
	{ 
		$("#txtArchivoSeguimiento").pekeUpload(
		{
			...
			url:				base_url+'proveedores/subirArchivosSeguimiento/<?php echo $idSeguimiento?>',
			maxSize:			25,
...
<!--<script src="'.base_url().'js/proveedores/seguimiento/archivosSeguimientoProveedores.js"></script>-->
	...
	foreach($archivos as $row)
	{
		...
		<tr '.$estilo.'>
			...
			<td>';
				...
				if(file_exists(carpetaSeguimientoProveedores.$row->idArchivo.'_'.$row->nombre) and strlen($row->nombre)>3)
				{	
					echo'<a title="Descargar" href="'.base_url().'proveedores/descargarArchivoSeguimiento/'.$row->idArchivo.'">'.$row->nombre.'</a>';
				}
				else
				{
			...
			<td align="center">
				<img id="btnBorrarArchivosSeguimiento'.$i.'" onclick="accesoBorrarArchivoSeguimientoProveedor('.$row->idArchivo.')" src="'.base_url().'img/borrar.png" width="22" title="Borrar fichero" /><br />
				<a id="a-btnBorrarArchivosSeguimiento'.$i.'">Borrar</a>
				'.($permiso[3]->activo==0?'
				<script>
					desactivarBotonSistema(\'btnBorrarArchivosSeguimiento'.$i.'\');
				</script>':'').'
			</td>
		</tr>';	
