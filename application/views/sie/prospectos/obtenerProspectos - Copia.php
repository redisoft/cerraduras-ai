...
<div class="row">
	<div class="col-md-6">
		<div class="card">
			...
			<div class="card-body">
				<canvas id="graficaProspectos" style="text-align: center" ></canvas>
			</div>
		</div>
	</div>
	<div class="col-md-6">
...
var barChartData = 
{
	labels: [<?php foreach($registros as $row){ ?> "<?php echo $row->grado?>",<?php }?>],
	datasets: [{
		label: 'Meta',
		backgroundColor: window.chartColors.red,
		data: [
			<?php foreach($registros as $row){ ?> "<?php echo $row->meta?>"  ,<?php }?>
		]
	}, {
		label: 'Resultado',
		...
		data: [
			...
			<?php foreach($registros as $row){ ?> "<?php echo $row->resultado?>"  ,<?php }?>
		]
	}]
...
$(document).ready(function()
{
	...
	/*var ctx = document.getElementById("graficaProspectos").getContext("2d");
	...
	window.myBar = new Chart(ctx, 
	{
		...
		options: 
		{
			title:{
				display:true,
				text:"Metas y resultados al <?=obtenerFechaMesLargo($fin,0)?>"
			},
			tooltips: {
				mode: 'index',
				intersect: false
