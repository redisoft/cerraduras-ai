...
<form id="frmProductoRecibido" name="frmProductoRecibido">
	...
	<table class="admintable" width="100%">
		...
		<tr>
			...
			<td>'.number_format($servicio->cantidad,2).'</td>
		</tr>
		<tr>
			<td class="key">Fecha entrega:</td>
			<td>'.obtenerFechaMesCorto($servicio->fechaEntrega,2).'</td>
		</tr>';
		...
		<tr>
			...
			<td>
				<input id="txtFechaRecibido" name="txtFechaRecibido" type="text" class="cajas" style="width:120px" value="'.date('Y-m-d H:i').'" />
				<script>
		...
		<tr>
			...
			<td>
				<input id="txtCantidadRecibir" 		name="txtCantidadRecibir"	type="text" class="cajas" style="width:100px" onkeypress="return soloDecimales(event)" maxlength="15"  />
				<input id="txtIdDetalle"   			name="txtIdDetalle"			type="hidden" value="'.$idDetalle.'" />
				<input id="txtIdComprita"   		name="txtIdComprita"		type="hidden" value="'.$servicio->idCompra.'" />
				<input id="txtIdProveedorCompra" 	name="txtIdProveedorCompra" type="hidden" value="'.$compra->idProveedor.'" />
			</td>
		</tr>
		<tr>
...
if($recibidos!=null)
{
	...
	foreach($recibidos as $row)
	{
		...
		<tr '.$estilo.'>
			...
			<td align="left">
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				<img src="'.base_url().'img/subir.png" width="22"  onclick="obtenerComprobantesCompras('.$servicio->idCompra.','.$row->idRecibido.')"  title="Comprobantes" />
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				<img id="btnBorrarRecibido'.$i.'" src="'.base_url().'img/borrar.png" width="20" title="Borrar" onclick="accesoBorrarServicioRecibido('.$row->idRecibido.')" /><br />
				<a>Comprobantes</a>
...
{
	echo '<div class="Error_validar">Aun no se han recibido el servicio</div>';
}