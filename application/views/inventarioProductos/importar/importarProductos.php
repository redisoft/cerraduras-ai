...
if($data->status == TRUE)
{
	...
	for($row = 3; $row <= $rows ; $row++)
	{
		...
		$producto['precioA']		= $precioImpuestos/(1+($impuesto/100));
		
		
		$producto['idDepartamento']	= $this->catalogos->registrarCatalogoNombre(trim($data->val($row,14)),'departamentos');
		$producto['idMarca']		= $this->catalogos->registrarCatalogoNombre(trim($data->val($row,15)),'marcas');
		
		$producto['stock']			= trim($data->val($row,16));
		$producto['idImpuesto']		= $this->configuracion->obtenerImpuestoTasa(trim($data->val($row,17)));
			
		$producto['idLinea']		= $this->importar->obtenerLinea(trim($data->val($row,12)));
		$producto['idSubLinea']		= $this->importar->obtenerSubLinea(trim($data->val($row,13)),$producto['idLinea']);
