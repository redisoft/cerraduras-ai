...
if(sistemaActivo=='cerraduras')
{
	...
	$promedio	= $this->inventario->obtenerCostoPromedioCompraProducto($idProducto);
}
...
<form id="frmEditarProducto" name="frmEditarProducto">
	<input type="hidden" id="txtIdPedimentoRegistro" name="txtIdPedimentoRegistro" value="'.$producto->idPedimento.'" />
	...
	<table class="admintable" width="100%">
		...
		<tr>
			<td class="key"><img src="'.base_url().'img/add.png" width="22" title="Agregar pedimento" onclick="obtenerCatalogoPedimentos()" /> Pedimento:</td>
			<td >
				<span id="lblPedimento">'.($producto->idPedimento==0?'Seleccione':$producto->pedimento).'</span>';

				
				echo '&nbsp;&nbsp;  <img id="btnBorrarPedimento" '.($producto->idPedimento==0?'style="display: none"':'').' src="'.base_url().'img/borrar.png" width="18" title="Borrar pedimento" onclick="borrarPedimentoProducto()" />';
		...
		<tr>
			<td class="key">Costo última compra:</td>
			<td>
				$'.number_format($ultimo,decimales).'
			</td>
		</tr>';
		...
		if(sistemaActivo=='cerraduras')
		{
			...
			<tr>
				...
				<td>
					<input type="text" class="cajas" name="txtCostoProducto" id="txtCostoProducto" style="width:20%" onkeypress="return soloDecimales(event)" maxlength="15" value="'.round($producto->costo,4).'"/>
				</td>
			</tr>';
		}
		...
		<tr>
			...
			<td>
				<input type="text" class="cajas" name="txtPrecioA" value="'.round($producto->precioA,4).'" id="txtPrecioA" style="width:20%" onkeypress="return soloDecimales(event)" maxlength="15"/>
			</td>
		</tr>
		...
		<tr>
			...
			if(sistemaActivo=='cerraduras')
			{
				echo ' &nbsp;&nbsp;&nbsp;&nbsp; A partir de: <input type="text" class="cajas" name="txtApartirB" id="txtApartirB" style="width:60px" onkeypress="return soloDecimales(event)" value="'.round($producto->cantidadMayoreo,4).'" maxlength="7"/>';
			}
		...
		<tr>
			...
			<td>
				...
				<input type="hidden" name="txtInventarioActual" value="'.round($producto->stock,4).'" id="txtInventarioActual" /> 
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				<input type="text" class="cajas" name="txtStockMinimo" id="txtStockMinimo" style="width:20%" onkeypress="return soloDecimales(event)" maxlength="15" value="'.round($producto->stockMinimo,4).'"/>
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				<input type="text" class="cajas" name="txtStockMaximo" id="txtStockMaximo" style="width:20%" onkeypress="return soloDecimales(event)" maxlength="15" value="'.round($producto->stockMaximo,4).'"/>
			</td>
		</tr>
	...
	if(sistemaActivo=='cerraduras')
	{
		...
		echo '
		<tr>
			<td class="key">Costo promedio:</td>
			<td>
				$'.number_format($promedio,decimales).'
			</td>
		</tr>
