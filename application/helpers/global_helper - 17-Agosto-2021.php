...
function facturaGlobal($configuracion,$cliente,$sello,$certificado,$fecha,$folio,$ventas)
{
	...
	foreach($ventas as $row)
	{
		...
		$descuento	= 0;#$importe*($row->descuentoPorcentaje/100);
		$descuento	= 0;#round($descuento,decimales);
		
		$diferencia	= 0;#$importe-$descuento;
		$diferencia	= 0;#round($diferencia,decimales);
	...
	foreach($ventas as $row)
	{
		...
		$XMLFacturaConceptos.=chr(13).chr(10).'    <cfdi:Concepto ClaveProdServ="01010101" Cantidad="'.Sprintf("% 01.2f",1).'" ClaveUnidad="ACT" Unidad="ACT" ValorUnitario="'.Sprintf("% 01.4f",$importe).'" Descripcion="'.sustituir($producto).'" Importe="'.Sprintf("% 01.2f",$importe).'">';
