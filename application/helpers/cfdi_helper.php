...
function xmlCfd($configuracion,$cliente,$productos,$sello,$certificado,$fecha,$folio,$cotizacion,$retenciones,$divisa,$venta=0)
{
	...
	#if($ivas>0)
	{
		...
		if($iva0)
		{
			$XMLFacturaImpuestos.=chr(13).chr(10).'      <cfdi:Traslado Base="'.Sprintf("% 01.2f",$subTotal0).'" Impuesto="002" TipoFactor="Tasa" TasaOCuota="0.000000" Importe="0.00"/>';
		}

		if($iva>0)
		{
			$XMLFacturaImpuestos.=chr(13).chr(10).'      <cfdi:Traslado Base="'.Sprintf("% 01.2f",$subTotal16).'" Impuesto="002" TipoFactor="Tasa" TasaOCuota="0.160000" Importe="'.Sprintf("% 01.2f",$iva).'"/>';
		}
