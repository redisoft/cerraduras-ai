...
function facturaGlobal($configuracion,$cliente,$sello,$certificado,$fecha,$folio,$ventas)
{
	...
	$XMLFacturaComprobante.=' Version="4.0" Folio="'.$folio.'" Serie="'.$configuracion->serie.'" Fecha="'.$fecha.'" Exportacion="01" FormaPago="'.$_POST['txtFormaPago'].'" NoCertificado="'.$configuracion->numeroCertificado.'" CondicionesDePago="'.$_POST['txtCondiciones'].'" SubTotal="'.Sprintf("% 01.2f",$subTotal).'"';
	
	
	$XMLFacturaComprobante.=' Moneda="MXN" TipoCambio="1" Total="'.Sprintf("% 01.2f",$total).'" MetodoPago="'.$_POST['txtMetodoPago'].'" TipoDeComprobante="I" LugarExpedicion="'.sustituir($configuracion->codigoPostal).'">';
	
	$XMLFacturaComprobante.=chr(13).chr(10).'<cfdi:InformacionGlobal Año="'.$_POST['selectAnio'].'" Meses="'.$_POST['selectMeses'].'" Periodicidad="'.$_POST['selectPeriodicidad'].'"/>';
	...
	$XMLFacturaReceptor=chr(13).chr(10).'  <cfdi:Receptor Rfc="'.espaciosFactura(sustituir($cliente->rfc)).'" Nombre="'.sustituir($cliente->razonSocial).'" UsoCFDI="S01" RegimenFiscalReceptor="'.sustituir($cliente->claveRegimen).'" DomicilioFiscalReceptor="'.sustituir($cliente->codigoPostal).'"/>';
	...
	$iva16	= 0;
	$iva0	= false; 
	$base	= 0; //ES LA BASE DE LOS IMPUESTOS
	...
	foreach($ventas as $row)
	{
		...
		$XMLFacturaConceptos.=chr(13).chr(10).'    <cfdi:Concepto ClaveProdServ="01010101" Cantidad="'.Sprintf("% 01.2f",1).'" ClaveUnidad="ACT" Unidad="ACT" ValorUnitario="'.Sprintf("% 01.4f",$importe+$importeSinIva).'" Descripcion="'.sustituir($producto).'" Importe="'.Sprintf("% 01.2f",$importe+$importeSinIva).'" ObjetoImp="02">';
		$XMLFacturaConceptos.=chr(13).chr(10).'<cfdi:Impuestos>';
		$XMLFacturaConceptos.=chr(13).chr(10).'<cfdi:Traslados>';
		
		if($importe>0)
		{
			$XMLFacturaConceptos.=chr(13).chr(10).'<cfdi:Traslado Base="'.Sprintf("% 01.2f",$importe).'" Impuesto="002" TipoFactor="Tasa" TasaOCuota="'.Sprintf("% 01.6f",$row->ivaPorcentaje/100).'" Importe="'.Sprintf("% 01.2f",$impuesto).'"/>';	
		}
		
		if($importeSinIva>0)
		{
			$iva0	= true;
			$XMLFacturaConceptos.=chr(13).chr(10).'<cfdi:Traslado Base="'.Sprintf("% 01.2f",$importeSinIva).'" Impuesto="002" TipoFactor="Tasa" TasaOCuota="'.Sprintf("% 01.6f",0).'" Importe="'.Sprintf("% 01.2f",0).'"/>';	
		}
	...
	if($ivas>0)
	{
		$XMLFacturaImpuestos.=chr(13).chr(10).'      <cfdi:Traslado Base="'.Sprintf("% 01.2f",$base).'" Impuesto="002" TipoFactor="Tasa" TasaOCuota="0.160000" Importe="'.Sprintf("% 01.2f",$ivas).'"/>';	
	}
