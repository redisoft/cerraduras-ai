	...
	$(document).on("click", ".ajax-pagReporte > li a", function(eve)
	{
		...
		$.ajax(
		{
			...
			data:
			{
				...
				fin: 				$('#txtFin').val(),
				contactado: 		$('#selectContactado').val(),
				cualificado: 		$('#selectCualificado').val(),
				idCausa: 			$('#selectEstatusCualificado').val(),
				interesado: 		$('#selectInteresado').val(),
				idDetalle: 			$('#selectDetallesCualificado').val(),
			},
			dataType:"html",
			beforeSend:function()
			{
...
function obtenerReporte()
{
	$.ajax(
	{
		...
		type:"POST",
		url:base_url+'crm/obtenerMetodoGlobal',
		data:
		{
			idFuente:			$('#selectFuentesBusqueda').val(),
			idPrograma: 		$('#selectProgramaBusqueda').val(),
...
function excelReporte()
{
	$.ajax(
	{
		...
		type:"POST",
		url:base_url+'crm/excelMetodoGlobal',
		data:
		{
			idFuente:			$('#selectFuentesBusqueda').val(),
			idPrograma: 		$('#selectProgramaBusqueda').val(),
		...
		success:function(data, textStatus)
		{
			...
			window.location.href=base_url+'reportes/descargarExcelReportes/'+data+'/MetodoGlobal';
