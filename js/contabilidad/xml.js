			...
			xhr.onreadystatechange = function()
			{
				if(xhr.readyState == 4 && xhr.status == 200)
				{
					...
					switch(data[0])
					{
						...
						case "1":
							notify('El comprobante se ha cargado correctamente',500,4000,'',30,5);
							
							obtenerDatosXml(file.name);
							/*//Póliza
...
function obtenerDatosXml(xml)
{
	$.ajax(
	{
		...
		{
			$('#registrandoInformacion').html('<label><img src="'+base_url+'img/loader.gif"/> El sistema esta procesando el comprobante cargado...</label>');
		},
		type:"POST",
		url:base_url+'contabilidad/obtenerDatosXml',
		data:
		{
			xml:		xml,
			fecha:		$('#txtFechaCatalogo').val(),
			tipoPoliza:	$('#selectTipo').val(),
			cobrada:	$('#selectTipoIngreso').val(),
			pagada:		$('#selectTipoEgreso').val(),
			idFactura:	0,
		...
		{
			$('#obtenerDatosXml').html(data);
			$('#registrandoInformacion').html('');
		},
		error:function(datos)
		{
			$('#registrandoInformacion').html('');
			notify("Error al procesar el comprobante",500,4000,"error"); 
		}
	});	
}

//PARA LOS COMPROBANTES QUE ESTEN ALOJADOS EN EL SISTEMA
function obtenerDatosXmlSistema(idFactura)
{
	$.ajax(
	{
		async:true,
